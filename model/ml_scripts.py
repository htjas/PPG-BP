...
def run_linear_regression(feat, ppg_train, abp_train, ppg_test, abp_test):
    lr_model = sklearn.linear_model.LinearRegression(n_estimators=100)
    mse, mae, r2, bias, loa = fit_predict_evaluate(lr_model, ppg_train, abp_train, ppg_test, abp_test)
...
def run_sv_regression(feat, ppg_train, abp_train, ppg_test, abp_test):
    ...
    mse, mae, r2, bias, loa = fit_predict_evaluate(svr_model, ppg_train, abp_train, ppg_test, abp_test)
...
def run_random_forest(feat, ppg_train, abp_train, ppg_test, abp_test):
    ...
    mse, mae, r2, bias, loa = fit_predict_evaluate(rf_model, ppg_train, abp_train, ppg_test, abp_test)
    logging.info(f'RF - MSE: {mse}, MAE: {mae}, R^2: {r2}, Bias: {bias}, LoA: {loa} ({feat})')


def fit_predict_evaluate(model, ppg_train, abp_train, ppg_test, abp_test):
